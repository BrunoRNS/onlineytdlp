...
help:
	...
	@printf "  MVN_EXTRA_PROFILES=,win-x86  Add extra Maven profiles\n"
...
$(BUILD_PACKAGE): $(TARGET_JAR_DESKTOP) $(BUILD_PYTHON) | $(BUILD_PACKAGE_DIR)
	...
	@exe=$$(find "$(TARGET_JPACKAGE_DIR)" -type f \( -name 'videodownloader.exe' -o -name 'videodownloader' \) -print -quit); \
	if [ -n "$$exe" ]; then \
		cp "$$exe" $@; \
		$(CHMOD) $@; \
	else \
		echo "jpackage output not found!"; \
		exit 1; \
	fi
